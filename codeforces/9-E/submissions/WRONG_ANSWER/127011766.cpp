...
int n, m, e, tot, d[N], f[N], v[N][N]; pair<int, int> ans[N];
...
int main(){
 ...
 int x=R, y=R; v[x][y]=v[y][x]=1, Merge(x, y);
 ...
 ++e, ++d[i], ++d[j], Merge(i, j), ans[++tot]=make_pair(i, j);
 if(m+e+1==n)
 for(reg int i=1; i<=n; ++i)
 for(reg int j=i+1; j<=n; ++j)
 if(d[i]<2&&d[j]<2) ++e, ++d[i], ++d[j], Merge(i, j), ans[++tot]=make_pair(i, j);
 for(reg int i=1; i<=n; ++i) if(Find(i)!=Find(i-1)) { puts("NO"); return 0; }
