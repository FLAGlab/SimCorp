...
int main() {
 ...
 if(u != v)par[u] = v ;
    		...
    		deg[x]++ , deg[y]++ ;
 if(deg[x] > 2 or deg[y] > 2)return puts("NO") , 0 ;
 }
    	std::vector< int > working ;
 for(int i = 1 ; i <= n; i++){
 if(deg[i] <= 1)working.pb(i);
 }
 if((yes and working.size()) or yes > 1)return puts("NO") , 0 ;
 if(!working.size() and yes == 1)return puts("YES\n0") , 0 ;
    	std::vector< pii > res ;
    	sort(all(working)) ;
 for(int i : working){
 if(deg[i] == 2)continue ;
 for(int j : working){
 if(deg[i] == 2 or deg[j] == 2)continue ;
