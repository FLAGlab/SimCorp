...
signed main() {
 ...
 auto lives = loadN<int>(treeNum);
 auto costs = loadN<int>(treeNum);
 auto lastRange = 0;
 auto newRange = 0;
 auto curr = vector<i64>(MAX_BIRDS, -INF);
 auto next = vector<i64>(MAX_BIRDS, -INF);
...
 
 ...
 if(curr[birds] != -INF) {
 auto canBuy = min(mana / single,(i64) lives[tree]);
