...
int main() {
 ...
 for (k = 1; k <= n[i]; k++)
 (ww = ((i-t[j])*X+w[j] < b[j] ? (i-t[j])*X+w[j] : b[j])-k*c[i]) >= 0 && (!t[j+k] || (w[j+k]+(i-t[j+k])*X < b[j+k] ? w[j+k]+(i-t[j+k])*X : b[j+k]) < ((i-t[j])*X+w[j] < b[j] ? (i-t[j])*X+w[j] : b[j])) && (w[j+k] = ww, b[j+k] = b[j]+B*k, t[j+k] = i, back < j+k && (back = j+k));
	printf("%lld\n", back);
}
