...
signed main() {
 ...
 for (k = 1; (i-t[j])*X+w[j]-k*c[i] >= 0 && k <= n[i]; k++)
				ww = ((i-t[j])*X+w[j] < b[j] ? (i-t[j])*X+w[j] : b[j])-k*c[i], ww >= 0 && (!t[j+k] || t[j] == t[j+k] && w[j+k] < ww) && (w[j+k] = ww, b[j+k] = b[j]+B*k, t[j+k] = i, back < j+k && (back = j+k));
	printf("%d\n", back);
}
