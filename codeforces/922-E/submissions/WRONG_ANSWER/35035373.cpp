...
signed main() {
 ...
 for (k = 1; k <= n[i]; k++)
 (ww = ((i-t[j])*X+w[j] < b[j] ? (i-t[j])*X+w[j] : b[j])-k*c[i]) >= 0 && (!t[j+k] || (w[j+k]+(i-t[j+k])*X < b[j+k] ? w[j+k]+(i-t[j+k])*X : b[j+k]) < ww) && (b[j+k] = b[j]+B*k, w[j+k] = ww < b[j+k] ? ww : b[j+k], t[j+k] = i, back < j+k && (back = j+k));
	printf("%d\n", back);
}
