...
bool xcorrdinateCompare(Point<ll> a,Point<ll> b)
{
 return a.x<b.x || (a.x==b.x && a.y>b.y);
}
...
 
 ...
 void convexupper(vector<Point<ll>> points,vector<Point<ll>> &edges)
{
 int n=points.size();
 Point<ll> start=makePoint((ll)INT_MAX,(ll)0);
 Point<ll> end=makePoint((ll)INT_MIN,(ll)0);
 for(int i=0;i<n;i++){
 ...
 else if(points[i].x==end.x && points[i].y>end.y)
      end=points[i];
 }
  sort(points.begin(),points.end(),xcorrdinateCompare);
  edges.pb(start);
 for(int i=1;i<n;i++){
 if(ccw(end,points[i],start)>0)continue;
 while(edges.size()>=2 && ccw(edges[edges.size()-1],points[i],edges[edges.size()-2])<0)
      edges.pop_back();
    edges.pb(points[i]);
 if(edges[i]==end)break;
 }
}
const int D=1e6+5;
signed main()
