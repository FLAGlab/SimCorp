...
int inmid(db k1,db k2,db k3){return sign(k1-k3)*sign(k2-k3)<=0;}// k3 �� [k1,k2] ��
struct point{
    db x,y;
...
point proj(point k1,point k2,point q){ // q ��ֱ�� k1,k2 ��ͶӰ
    point k=k2-k1; return k1+k*(dot(q-k1,k)/k.abs2());
}
point reflect(point k1,point k2,point q){return proj(k1,k2,q)*2-q;}
int clockwise(point k1,point k2,point k3){// k1 k2 k3 ��ʱ�� 1 ˳ʱ�� -1 ���� 0
 return sign(cross(k2-k1,k3-k1));
}
int checkLL(point k1,point k2,point k3,point k4){// ��ֱ�� (L) �߶� (S)k1,k2 �� k3,k4 �Ľ���
...
vector<line> getHL(vector<line> &L){ // ���ƽ�潻 , ��ƽ������ʱ�뷽�� , ���������ʱ��
...
db closepoint(vector<point>&A,int l,int r){ // ������ , ��Ҫ���� x ��������
 if (r-l<=5){
        db ans=1e20;
...
int checkposCC(circle k1,circle k2){// ��������Բ�Ĺ���������
...
vector<point> TangentCP(circle k1,point k2){// ��Բ k1 ��ʱ�����
...
db area(vector<point> A){ // ������� vector<point> ��ʾ , ��ʱ��
    db ans=0;
...
int contain(vector<point>A,point q){ // 2 �ڲ� 1 �߽� 0 �ⲿ
...
vector<point> ConvexHull(vector<point>A,int flag=1){ // flag=0 ���ϸ� flag=1 �ϸ�
...
int checkPosFast(vector<point>A,point k1,point k2){ // ���ټ���߶��Ƿ�Ͷ�����ϸ��ཻ
...
VP getFF(P3 p1,P3 dir1,P3 p2,P3 dir2){// ����һ����
...
int main(){
 ...
 for(int i=0;i<n;i++){
    ...
    getUDP(t,u,d);
 /*
    cout<<t.size()<<"  "<<u.size()<<"  "<<d.size()<<endl;
    for(int i=0;i<t.size();i++){
        printf("%f %f %f\n",t[i].x,t[i].y+t[i].x*t[i].x,t[i].y);
    }*/
 int ans=u.size()-1;
